...
public IntakeSubsystem(rollerIO io, IntakeIO pivotIo){
    ...
    pivotDisconnectedAlert = 
        new Alert("Disconnected pivot motor in intake", AlertType.kError);
        rollerDisconnectedAlert = 
        new Alert("Disconnected roller motor in intake", AlertType.kError);
}
...
//research command syntax and then check back with me (Finn), when you think you have code that works.
//you will need to return something that isn't null. That's just a placeholder so that there are no errors in code.
